...
export default async function handler(req: any, res: any) {
  ...
  try {
    ...
    const result = await supabaseGet(
      `players?player_id=eq.${encodeURIComponent(playerId)}&select=player_id,full_name,application_status,payment_status,position,area,age,photo_url,created_at&limit=1`,
      timeout.signal,
    );
    timeout.clear();
